...
def run_vae_pipeline(dataset_name: str, vae_type: str):
    ...
    appliance_id = [0, 1, 4, 5, 6, 7]
    data = np.load(r"C:\Users\ASUS\Desktop\final_trial\general_cluster\nilm_3\0.0.npy")[:64] # 682，1440，6
    train_ori_dataset = data
    # train_ori_dataset = train_ori_dataset[:, :, appliance_id]  # 3,1440,6
    in_channel= data.shape[2]

    # load_profiles = torch.from_numpy(train_ori_dataset)

    train_ori_dataset = train_ori_dataset
    from sklearn.preprocessing import StandardScaler
    scaler = StandardScaler()
    # train_ori_dataset = scaler.fit_transform(train_ori_dataset.reshape(-1,in_channel))
    train_ori_dataset = scaler.fit_transform(train_ori_dataset.reshape(-1, 1))
    train_ori_dataset = train_ori_dataset.reshape(-1, in_channel)

    def seq_dataset(x, width, stride, in_ch):
        x_ = []
        for t in range(0, x.shape[0] - width, stride):
            x_.append(x[t:t + width])
        x_ = np.array(x_).reshape(-1, width, in_ch)
        return x_

    batch_size = 32
    train_ori_dataset = seq_dataset(train_ori_dataset.reshape(-1, in_channel), 1440, 120, in_channel)
    scaled_train_data = train_ori_dataset
    # ----------------------------------------------------------------------------------
    ...
    def visualize_hidden_space(vae, scaled_train_data):
        ...
        np.save(r'C:\Users\ASUS\Desktop\timeVAE-main\outputs\feature_map\input_data.npy',data.reshape(data.shape[1],data.shape[2]))
        plt.show()
        # plt.title("appliance load")
        plt.show()
        ...
        np.save(r'C:\Users\ASUS\Desktop\timeVAE-main\outputs\feature_map\enc_conv_1.npy',enc_conv_1.reshape(43,50))

        plt.show()
        plt.imshow(enc_conv_2.reshape(22,100))
        # plt.title("enc_conv_2")
        np.save(r'C:\Users\ASUS\Desktop\timeVAE-main\outputs\feature_map\enc_conv_2.npy',enc_conv_2.reshape(22,100))

        plt.show()
        plt.imshow(enc_conv_3)
        # plt.title("last_conv")
        plt.show()
        np.save(r'C:\Users\ASUS\Desktop\timeVAE-main\outputs\feature_map\enc_conv_3.npy',enc_conv_3)
        ...
        plt.show()
        np.save(r'C:\Users\ASUS\Desktop\timeVAE-main\outputs\feature_map\level_para_final.npy',level_para_final)
        ...
        plt.show()
        np.save(r'C:\Users\ASUS\Desktop\timeVAE-main\outputs\feature_map\dec_deconv_0.npy',dec_deconv_0.reshape(22,100))

        plt.imshow(dec_deconv_1.reshape(44,50))
        # plt.title("dec_deconv_1")
        plt.show()
        np.save(r'C:\Users\ASUS\Desktop\timeVAE-main\outputs\feature_map\dec_deconv_1.npy',dec_deconv_1.reshape(44,50))

        plt.imshow(dec_deconv__2.reshape(5,88))
        # plt.title("dec_deconv__2")
        plt.show()
        np.save(r'C:\Users\ASUS\Desktop\timeVAE-main\outputs\feature_map\dec_deconv__2.npy',dec_deconv__2.reshape(5,88))

        plt_data(residual_reshape)
        np.save(r'C:\Users\ASUS\Desktop\timeVAE-main\outputs\feature_map\residual_reshape.npy',residual_reshape)
        ...
        np.save(r'C:\Users\ASUS\Desktop\timeVAE-main\outputs\feature_map\output.npy',output.reshape(85,5))
    ...
    x_decoded = scaler.inverse_transform(x_decoded.reshape(-1,1)).reshape(-1,1440,1)
    ...
    prior_samples = scaler.inverse_transform(prior_samples.reshape(-1,1)).reshape(-1,1440,1)
