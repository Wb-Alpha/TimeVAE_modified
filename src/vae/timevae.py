...
class TimeVAE(BaseVariationalAutoencoder):
    ...
    def _get_refined_encoder(self):
        encoder_inputs = Input(
            shape=(self.seq_len, self.feat_dim), name="encoder_input"
        )
        x = encoder_inputs
        x = Reshape(target_shape=(60, 48, 6), name="enc_reshape")(
            x
        )

        x = Conv2D(256, kernel_size=(3, 2), strides=(2, 1), padding="same", name="enc_conv_0")(x)
        x = LeakyReLU(0.2)(x)
        x = Conv2D(256, kernel_size=(3, 2), strides=(2, 1), padding="same", name="enc_conv_1")(x)
        x = LeakyReLU(0.2)(x)
        x = Conv2D(64, kernel_size=(3, 2), strides=(2, 1), padding='same', name="enc_conv_2")(x)
        x = LeakyReLU(0.2)(x)
        # self.encoder_shape_0 = x.shape[-0]
        self.last_2_layer = x.shape
        print(x.shape)
    ...
    def _get_refined_decoder(self):
        decoder_inputs = Input(shape=(self.latent_dim,), name="decoder_input")
        x = decoder_inputs

        # 将潜在向量转换为适合卷积操作的形状
        x = Dense(self.encoder_last_dense_dim, activation='relu')(x)
        x = Reshape(target_shape=(8, 48, 64), name="dec_reshape")(
            x
        )
        # 解码器中的反卷积层
        x = Conv2DTranspose(64, kernel_size=(3, 2), strides=(2, 1), padding='same', activation='relu',
                            name="dec_conv_transpose_0")(x)
        x = LeakyReLU(0.2)(x)

        x = Conv2DTranspose(256, kernel_size=(3, 2), strides=(2, 1), padding='same',
                            name="dec_conv_transpose_1")(x)
        x = LeakyReLU(0.2)(x)

        x = Conv2DTranspose(256, kernel_size=(3, 2), strides=(2, 1), padding='same',
                            name="dec_conv_transpose_2")(x)
        x = LeakyReLU(0.2)(x)

        x = Conv2DTranspose(6, kernel_size=(3, 2), strides=(2, 1), padding='same', activation='tanh',
                            name="dec_conv_transpose_3")(x)

        x = Flatten(name="dec_flatten")(x)
        x = Dense(self.seq_len * self.feat_dim, name="decoder_dense_final",activation='relu')(x)
        x = Reshape(target_shape=(self.seq_len, self.feat_dim), name='residual_reshape')(x)
    ...
    def level_model(self, z):
        level_params = Dense(self.feat_dim, name="level_params1", activation="relu")(z)
        level_params = Dense(self.feat_dim, name="level_params2")(level_params)
        level_params = Reshape(target_shape=(1, self.feat_dim), name = "level_params_feature")(
            level_params
        )  # shape: (N, 1, D)
        ...
        level_vals = tf.keras.layers.Lambda(
            lambda x: x[0] * x[1],
            name="level_param_final"  # 设置名字
        )([level_params, ones_tensor])
    ...
    def _get_decoder_residual(self, x):
        ...
        residuals = Reshape(target_shape=(self.seq_len, self.feat_dim), name= 'residual_reshape')(x)
        return residuals
