...
class BaseVariationalAutoencoder(Model, ABC):
    ...
    def get_prior_samples(self, num_samples):
        Z = np.random.randn(num_samples, self.latent_dim) # 79,8
        # level_para_1 = tf.keras.models.Model(inputs=self.decoder.input, outputs=self.decoder.get_layer('level_params').output) # 注意
        # level_para_2 = tf.keras.models.Model(inputs=self.decoder.input, outputs=self.decoder.get_layer('level_params2').output) # 注意
        # dec_dense = tf.keras.models.Model(inputs=self.decoder.input, outputs=self.decoder.get_layer('dec_dense').output)
        # level_para_1_output = level_para_1.predict(x=Z, verbose=0) #791,5
        # dec_deconv_0
        # dec_deconv_1
        # dec_deconv_2
        # dec_deconv_3
        ...
        # print(level_para_1_output.shape)
        return samples
